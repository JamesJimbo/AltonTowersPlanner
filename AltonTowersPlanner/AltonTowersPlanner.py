...
def chooseStartingCoaster(rollerCoasters):
    ...
    print("r. Random Coaster")

    while True:
        choice = input("\nEnter The Number of The Coaster or 'r' For a Random Coaster: ")
        if choice.lower() == 'r':
            return random.choice(coasterList)
        elif choice.isdigit():
            index = int(choice) - 1
            if 0 <= index < len(coasterList):
                return coasterList[index]
        print("Invalid choice. Please enter a valid number or 'r' for a random coaster.")
